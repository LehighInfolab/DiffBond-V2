...
def parseArg():
    ...
    parser.add_argument(
        "-i", nargs=2, metavar="InputPDB", help="Input PDB file to be compared."
    )
    parser.add_argument(
        "-d",
        nargs="?",
    ...
    parser.add_argument(
        "-o", nargs="?", metavar="OutputPDB", help="Output PDB file name"
    )
...
def writeContactList(
    ...
    for i in write_list:
        ...
        for j in i[1]:
            a = (
                str(splitLines_PDB2[int(j)][4])
                + " "
                + str(splitLines_PDB2[int(j)][3])
                + str(splitLines_PDB2[int(j)][5])
            )
            dupe_list.append(a)
    ...
    f.close
    print(
        "Contact list written to ContactList_"
        + outputFileName
        + "_"
        + str(dist)
        + ".pdb"
