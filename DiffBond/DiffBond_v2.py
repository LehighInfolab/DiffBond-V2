...
def compareDistIonic(points1, points2, dist):
	...
	for edge in temp:
		...
		if charge1 == charge2:
			continue
		# if swapped:
		#     temp_edge = [edge[1], edge[0], edge[2]]
		#     output.append(temp_edge)
		else:
			output.append(edge)
...
def HB_processing(i_list, edges, outputFileName):
	...
	try:
		hb_file_name = outputFileName.split("/")[-1] + ".txt"
...
def c_mode(PDB_data, dist, results_dir, use_visual, verbose = False):
...
def i_mode(PDB_data, dist, results_dir, use_visual, verbose = False):
...
def h_mode(PDB_data, dist, results_dir, use_visual, i_list, outputFileName, verbose = False):
...
def a_mode(PDB_data, dist, results_dir, use_visual, verbose = False):
...
def s_mode(i_graph, h_graph, results_dir, use_visual, verbose = False):

	if i_graph == None or h_graph == None:
		return

	print("##### Merging ionic graph and hydrogen bond graph #####")
...
def main():
	...
	# Directory in results dir to store pdbs for next step in pipeline. If pdb names are the same, add 1 and 2 to pdbs respectively.
	pdb_dir = make_pdb_dir(outputFileName)
	copy_list= i_list
	index = 1
	for i in copy_list:	
		dest_path = os.path.join(pdb_dir, str(index) + ".pdb")
		index = index + 1
		try:
