...
def queryBoxOnAllDonors(donor_hydrogen_list, PDB_file):
    ...
    for donor_hydrogen_pair in donor_hydrogen_list:
        ...
        max_y_block_forRange, min_y_block_forRange = add_buffer_block_in_y_direction(
            max_y_block_forRange, min_y_block_forRange
        )
        max_z_block_forRange, min_z_block_forRange = add_buffer_block_in_z_direction(
    ...
    for pair in potential_H_bonds:
        ...
        if donor_atom.is_disordered() != 0:
            warnings_list.append(
                ...
                + donor_atom.get_name()
                + " in "
                + donor_atom.get_parent().get_resname()
                + " is a disordered atom. Could lead to incorrect hydrogen bonds."
            )
        if acceptor_atom.is_disordered() != 0:
            warnings_list.append(
                "WARNING: "
                + acceptor_atom.get_name()
                + " in "
                + acceptor_atom.get_parent().get_resname()
                + " is a disordered atom. Could lead to incorrect hydrogen bonds."
            )
...
def printHBonds_Table(listOfHBonds, listOfHBondInformation, PDB_File):
    ...
    PDB_File = os.path.splitext(os.path.basename(PDB_File))[0]
